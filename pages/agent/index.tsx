...
const AgentList: NextPage = ({ initialInput, ...props }: any) => {
	...
	const likeMemberHandler = async (user: any, id: string) => {
		try {
			if (!id) return; // qaysi memberga like bosmoqchimiz
			if (!user._id) throw new Error(Messages.error2); // like qilayotgan user auth bolganmi?s
			...
			await getAgentsRefetch({ input: searchFilter }); // refetch searchFilet ichida ==> page limit search bor
